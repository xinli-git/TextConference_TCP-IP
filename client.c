...
void parse_login(char** addr, char** port, char** username, char** password);
...
int main(void) {
    ...
    parse_login(&addr, &port, &username,&pswd);
    ...
    strcpy(userRequest.data,pswd);
    userRequest.size=sizeof(userRequest.data);
    strcpy(userRequest.source,username);
    ...
    if (serverResponse.cmd==LO_ACK){
        printf("successful login, entering login mode\n");
